...
import com.tc_awc.utilities.ExtentFactory;
...
public class ActionEngine {
	...
	// Select dropdown value value by index number
	public void selectDropDownByIndex_custom(WebElement element, String fieldName, int indexNumber) throws Throwable {
		try {
			Select s = new Select(element);
			s.selectByIndex(indexNumber);
			ExtentFactory.getInstance().getExtent().log(Status.PASS,
					fieldName + "==> Dropdown Value Selected by index : " + indexNumber);
		} catch (Exception e) {
			ExtentFactory.getInstance().getExtent().log(Status.FAIL,
					"Dropdown value not selected for field: " + fieldName + "  due to exception: " + e);
		}
...
//				Assert.assertTrue(false);
	...
	public void assertNotEqualsString_custom(String expvalue, String actualValue, String locatorName) throws Throwable {
		try {
			if (actualValue.equals(expvalue)) {
				ExtentFactory.getInstance().getExtent().log(Status.FAIL, "String Assertion is successful on field "
						+ locatorName + " Expected value was: " + expvalue + " actual value is: " + actualValue);
			} else {
				ExtentFactory.getInstance().getExtent().log(Status.PASS, "String Assertion FAILED on field "
						+ locatorName + " Expected value was: " + expvalue + " actual value is: " + actualValue);
	...
	public String getprint_custom(String text, String fieldName) {
		// String text = "";
		try {
			ExtentFactory.getInstance().getExtent().log(Status.INFO, fieldName + "==> Text : " + text);
			return text;
		} catch (Exception e) {
			ExtentFactory.getInstance().getExtent().log(Status.INFO, fieldName + "==> Text : " + e);
